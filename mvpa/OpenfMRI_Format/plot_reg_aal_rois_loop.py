...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 25 18:48:37 2021
...
#analysis_name_list = ['food','trinket']
#analysis_name_legend = ['Food','Trinket']

analysis_name_list = ['abs_value']
analysis_name_legend = ['Absolute Value']

df_data = pd.DataFrame({'Mask':[0], 'Trial Type':[0], 'Accuracy':[0], 'Subj': [0]})
...
for analysis_name in analysis_name_list:
    ...
    for subj in subj_list:
        ...
        for mask in mask_loop:
            score = temp_scores[mask]
            df_data = df_data.append({'Mask': mask, 'Trial Type': analysis_name_legend[analysis_count], 'Accuracy': score, 'Subj': subj}, ignore_index=True)
            mask_count+=1
    analysis_count+=1
df_data = df_data.drop([0])

plt.figure(figsize=(10, 5))
ax = sns.barplot(x='Mask', y='Accuracy', hue='Trial Type', data=df_data, ci=68)
