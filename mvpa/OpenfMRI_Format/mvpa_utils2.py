...
def make_targets(subj, glm_ds_file, mask_name, conditions, presave_file, relative_value, tolman=False):
    
    if os.path.isfile(presave_file):
        print 'Loading presaved y data'
        fds_list = h5load(presave_file)
        fds_x, fds_y, fds_chunks, fds_trial_categ, fds_vox_inds = fds_list
    else:
        ...
        #temp
        #separately save and load all components to make more lightweight
        fds_x = fds_subset.samples
        fds_y = fds_subset.targets
        fds_chunks = fds_subset.chunks
        fds_trial_categ = fds_subset.sa.trial_categ
        fds_vox_inds = fds.fa.voxel_indices
        
        #save this data for next time
        y_list = [fds_x, fds_y, fds_chunks, fds_trial_categ, fds_vox_inds]
        h5save(presave_file,y_list)
        
        print 'Finished making targets',time.time() - start_time
    
    return fds_x, fds_y, fds_chunks, fds_trial_categ, fds_vox_inds
