# -*- coding: utf-8 -*-
"""
Created on Tue Mar 28 09:26:55 2023
...
#save_file = 'rsa_div_vs_sub'
save_file = 'rsa_norm_model_codes_range'
...
# file_column_keys = [('rsa_div_rw_mc_norm_sweeps', 'RW Update'),
#                     ('rsa_div_rw_mc_norm_sweeps', 'MC'),
#                     ('rsa_div_rw_mc_norm_sweeps', 'Divisive'),
#                     ('rsa_normalized_codes', 'Relative')]

# model_labels = ['RW Update', 'MC', 'Divide Mean', 'Z-Score']
...
for i,subj in enumerate(subj_list):
    ...
    subj_plot = f"{int(subj)-100:03d}"
    sns.despine()
    #plt.ylabel('RSA Correlation (r)', fontsize=18)
    plt.ylabel(y_label, fontsize=18)
    plt.xlabel('Model', fontsize=16)
    if len(mask_names) > 1:
        legend = plt.legend(bbox_to_anchor=(1.0, 1),borderaxespad=0, loc="upper left")
        #legend.remove()
        plt.title('Sub'+subj_plot+' RSA Normalized Codes', fontsize=18)
    else:
        #plt.xticks(np.arange(len(models)), model_labels, rotation=0)
        plt.xticks(rotation=45)
        plt.title('Sub'+subj_plot+' RSA Normalized Codes in '+mask_names[0], fontsize=18)
    if save:
        temp_file = 'sub'+subj+'_'+save_file
        plt.savefig(bundle_path+'figures/individual_subj_plots/'+temp_file, dpi=500, bbox_inches='tight')
    plt.show()