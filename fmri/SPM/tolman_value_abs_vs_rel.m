function tolman_value_abs_vs_rel(subID)
...
if relative_value
    spm_dir = ['/home/lcross/Bundle_Value/analysis/SPM/sub',subID,'/rel_value'];
else
    spm_dir = ['/home/lcross/Bundle_Value/analysis/SPM/sub',subID,'/abs_value'];
end
...
for day=1:3
    ...
    %zscore value by category for relative value
    sitem_inds = find(trial_cat == 0);
    bundle_inds = find(trial_cat == 1);
    if relative_value
        trial_value(sitem_inds) = zscore(trial_value(sitem_inds));
        trial_value(bundle_inds) = zscore(trial_value(bundle_inds));
    end
