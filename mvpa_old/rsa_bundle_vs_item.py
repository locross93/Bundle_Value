...
for subj in subj_list:
    ...
    fmri2model_matrix = np.zeros([len(mask_loop), 4])
    ...
    for conditions in conditions_list:
        ...
        for mask_num in range(len(mask_loop)):
            for model_num in range(len(model_dsm_names)):
                ...
                fmri2model_matrix[mask_num, model_num+(2*count)] = temp_correl
        count+=1
        
    xaxis_labels = ['ivalue','ichoice','bvalue','bchoice']
    if ranked:
    ...
    if save:
        ...
        subj_df.to_csv(save_path+'rsa_bundle_vs_item.csv')
