...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 23 22:03:38 2022
...
#            mask_name = '/Users/logancross/Documents/Bundle_Value/mvpa/OpenfMRI_Format/sub'+str(subj)+'/masks/'+mask+'.nii.gz'
        ...
        cv_score_s2sbn = np.zeros([run_num])
        cv_score_b2abs = np.zeros([run_num])
        cv_score_b2rel = np.zeros([run_num])
        cv_score_b2sbn = np.zeros([run_num])
        ...
        for train, test in gkf.split(X, abs_value, groups=cv_groups):
            ...
            cv_score_s2sbn[cv_count] = get_correlation(y_preds_test,subnorm_value[test])
            ...
            cv_score_b2sbn[cv_count] = get_correlation(y_preds_test,subnorm_value[test])
        ...
        print 'Train single, subtractive value:',np.mean(cv_score_s2sbn)
        print 'Train bundle, absolute value:',np.mean(cv_score_b2abs)
        print 'Train bundle, relative value:',np.mean(cv_score_b2rel)
        print 'Train bundle, subtractive value:',np.mean(cv_score_b2sbn)
        print '\n'
        
        mask_scores = [subj, mask_label, np.mean(cv_score_s2abs), np.mean(cv_score_s2rel), np.mean(cv_score_s2sbn), 
                       np.mean(cv_score_b2abs), np.mean(cv_score_b2rel), np.mean(cv_score_b2sbn)]
    ...
    if save:
        ...
        np.save(save_path+'/xdecode_norm_codes_scores.npy', subj_scores)
        
df_data = pd.DataFrame(data_list, columns = ['Subj', 'Mask','S2Abs','S2Rel','S2Sbn','B2Abs','B2Rel','B2Sbn']) 
...
#    if not os.path.isdir(save_path):
#        os.makedirs(save_path)
#    np.save(save_path+'/xdecode_norm_codes_scores_lowres_subs.npy', df_plot)
#
