...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 11 14:14:12 2022
...
pairs = []
for mask in mask_names:
    temp_pair = ((mask, 'Absolute'),(mask, 'Relative'))
    pairs.append(temp_pair)

hue_order = ["Absolute", "Relative"]
order = mask_names
ax = sns.barplot(x='ROI', y='value', hue='variable', data=df_plot, order=order, hue_order=hue_order, ci=68)
sns.despine()
annot = Annotator(ax, pairs, data=df_plot, x='ROI', y='value', hue='variable', order=order, hue_order=hue_order,)
...
# sig_bools, corr_pvals, alphacSidak, alphacBonf = multipletests(uncorr_pvals, alpha=0.05, method='fdr_bh')
...
#         print(mask,' Significant ',cat_diff_list[mask_num],uncorr_pvals[mask_num])
