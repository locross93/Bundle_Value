...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 15 15:14:45 2022
...
for subID in subs2test:
    ...
    res_item = smf.logit(formula='Choice ~ Item_Value + Ref_Amount', data=df_item_choice).fit()        
    ...
    res_bun = smf.logit(formula='Choice ~ Bundle_Value + Ref_Amount', data=df_bundle_choice).fit()        
    ...
    res_bun = smf.logit(formula='Choice ~ Item_Val1 + Item_Val2 + Ref_Amount', data=df_bundle_choice).fit()        
    print res_bun.summary()
    pred_table = res_bun.pred_table()
