...
func_path=/home/lcross/Bundle_Value/preprocessing/FSL_ICA_BET/sub${sbj}/Session${ses}.ica
...
fslroi ${func_path}/filtered_func_data_clean_unwarped_new.nii.gz ${func_path}/filtered_func_data_sample_1.nii.gz 0 1

# added by Eva, because with eyetracking head move is so close to the coil
fslmaths ${func_path}/filtered_func_data_clean_unwarped_new.nii.gz -Tmean ${func_path}/filtered_func_mean_clean.nii.gz

# do a voxel-value bia correction
echo "Running bias correction at $(date +"%T")"
/usr/local/ANTs/build/bin/N4BiasFieldCorrection -i ${func_path}/filtered_func_mean_clean.nii.gz -o ${func_path}/filtered_func_data_bias.nii.gz --convergence [50x50x30x20,0.0] -d 3 -v 1
...
echo "Apply series of transformations all the way from func to lowres atlas (in MNI space)"
/usr/local/ANTs/build/bin/WarpTimeSeriesImageMultiTransform 4 ${func_path}/filtered_func_data_clean_unwarped_new.nii.gz ${func_path}/filtered_func_data_clean_unwarped_new_ANTsReg.nii.gz \
	-R ${anat_path}/CIT168_T2w_MNI_lowres.nii.gz \
	${func_path}/itk_transformation_hires_to_lowres.txt \
    ${outPrefix}_xfm1Warp.nii.gz ${outPrefix}_xfm0GenericAffine.mat \
    ${func_path}/itk_transformation_func_to_struct.txt
